...
$userUploadPhotoModel = new \User\Model\UserRestaurantimage();
...
$joins [] = array(
    ...
    'on' => new Expression("(user_restaurant_image.restaurant_id = r.id)"),
    'columns' => array(
...
$joins [] = array(
    ...
    'on' => new Expression("(user_restaurant_image.user_id = u.id)"),
...
$options = array(
    ...
    'joins' => $joins,
    'where' => array('(user_restaurant_image.status = "0" OR  user_restaurant_image.status = "1") and user_restaurant_image.cronUpdate="0" and DATE_FORMAT( user_restaurant_image.updated_on,  "%Y-%m-%d" ) = CURDATE( ) '),
    'order'=>'id DESC'
);
$userUploadPhotoModel->getDbTable()->setArrayObjectPrototype('ArrayObject');
$allPhotos = $userUploadPhotoModel->find($options)->toArray();
//pr($allPhotos,1);
if(count($allPhotos) > 0){
    foreach($allPhotos as $key=>$values){
        ...
        if(trim($values['status'])=='0'){
            if($values['user_id']>0){
             $feed = array(
                ...
                $activityFeed = addActivityFeed($feed, 72, $replacementData, $otherReplacementData,$values['restaurant_id']);      
                $userUploadPhotoModel->updateCronOrder($values['id']);
            }
        }
        //status 0 =approved 
        if(trim($values['status'])=='1'){
            if($values['user_id']>0){
             $feed = array(
                ...
                $activityFeed = addActivityFeed($feed, 71, $replacementData, $otherReplacementData,$values['restaurant_id']);
                $userUploadPhotoModel->updateCronOrder($values['id']);
            }
        }
